...
URL_CANDIDATES = ["url_o", "photo_image_url", "photo_url", "url", "download_url"]
...
def parse_wh(s):
    ...
    if not m:
        raise ValueError("resize must look like 640x480")
...
def dl_resize(row, out_dir, wh):
    ...
    if jpg_path.exists():
        return True
    try:
        r = requests.get(row.url_o, timeout=10)
        r.raise_for_status()
...
def main(a):
    ...
    with cf.ThreadPoolExecutor(max_workers=a.threads) as ex:
        for res in cf.as_completed(
            [ex.submit(dl_resize, r, out_dir, wh) for r in df.itertuples()]
        ):
            ok += res.result()
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--csv_glob", default="backgrounds/photos.csv*", help="glob for Unsplash files"
    )
